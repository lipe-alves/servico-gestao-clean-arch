...
@Injectable()
class AssinaturaServico extends ServicoBase<AssinaturaEntidade, AssinaturaModelo> {
  ...
  public async atualizar(id: number, dados: Partial<AssinaturaEntidade>): Promise<AssinaturaModelo> {
    if (typeof dados.custoFinal === "number") {
      if (dados.custoFinal <= 0) throw new Error("O custo final não pode ser igual ou menos que zero!");
    }

    return super.atualizar(id, dados);
  }

  public async criar(dados: Omit<AssinaturaEntidade, 'codigo' | 'dataUltimoPagamento'>): Promise<AssinaturaModelo> {
    if (dados.custoFinal <= 0) throw new Error("O custo final não pode ser igual ou menos que zero!");
