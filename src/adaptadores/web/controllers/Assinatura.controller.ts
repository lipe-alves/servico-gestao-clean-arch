...
import { BuscarAssinaturasDto, validarBuscarAssinaturasDto } from 'src/aplicacao/dtos/assinaturas/BuscarAssinaturas.dto';
...
class AssinaturaController extends ControllerBase {
  ...
  public async getAssinatura(
    ...
    try {
      ...
      if (ehNumerico(statusOuCodigo)) {
        ...
        const [assinatura] = await this.buscarAssinaturasCasoUso.executar(params);
        return this.sucesso('Consulta realizada com sucesso!', assinatura.paraJson());
      }
      
      params.status = statusOuCodigo;
  ...
  public async getAssinaturasCliente(
    ...
    try {
      const assinaturas = await this.buscarAssinaturasCasoUso.executar({ codCliente });
  ...
  public async getAssinaturasPlano(
    ...
    try {
      const assinaturas = await this.buscarAssinaturasCasoUso.executar({ codPlano });
  ...
  public async postAssinatura(
    ...
    try {
      ...
      return this.sucesso('Cadastro efetuado com sucesso!', assinatura.paraJson());
    } catch (err) {
      return this.falha(err);
    }
  }
  ...
  public async patchAssinatura(@Param("idAssinatura") id, @Body() dados) {
    try {
      ...
      const assinatura = await this.atualizarAssinaturaCasoUso.executar(id, dados);
      return this.sucesso('Atualizção efetuada com sucesso!', assinatura.paraJson());
    } catch (err) {
      return this.falha(err);
    }
  }
